...
// Checking if conversation has been made
 ...
 $_POST = json_decode(file_get_contents('php://input'));
  ...
  $json_data = isset($_POST->message) ? $_POST->message : '';
   ...
   //who are you loading this api, oya start dey go our main branch
    header("location:../");
...
if($verifyUrl === true){//this request is comming from our bot, process
 if($verifyEmail === true){//bot response email is already one of our users
   ...
   //who are you loading this api with post method but nut bot controller, oya start dey go our main branch
    header("location:../");
    return false;//do not process any other thing, kick them off this place
}
